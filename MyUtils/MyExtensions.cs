...
namespace PRADA_Vayne.MyUtils
{
    public static class Extensions
    {
        ...
        public static bool IsCondemnable(this AIHeroClient hero)
        {
            ...
            if (mode == "PRADASMART" && (p.Extend(pP, -pD).IsCollisionable() || p.Extend(pP, -pD / 2f).IsCollisionable() || p.Extend(pP, -pD / 3f).IsCollisionable()))
            {
                ...
                if (enemiesCount > 1 && enemiesCount <= 3)
                {
                    ...
                    {
                        var posFlags = NavMesh.GetCollisionFlags(prediction.UnitPosition.To2D().Extend(pP.To2D(), -i).To3D());
                ...
                for (var i = 15; i < pD; i += 100)
                    if (pP.To2D().Extend(alpha, i).To3D().IsCollisionable() && pP.To2D().Extend(beta, i).To3D().IsCollisionable())
                        return true;
                return false;
            }
        ...
        {
            return HeroManager.Enemies.Any(e => e.IsValidTarget() && e.IsVisible && e.Distance(pos) < Program.ComboMenu.Item("QMinDist").GetValue<Slider>().Value) || Traps.EnemyTraps.Any(t => pos.Distance(t.Position) < 125) || pos.UnderTurret(true) && !Player.UnderTurret(true) || pos.IsWall();
        }
        ...
        {
            return NavMesh.GetCollisionFlags(pos).HasFlag(CollisionFlags.Wall) || Program.Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Combo && NavMesh.GetCollisionFlags(pos).HasFlag(CollisionFlags.Building);
        }
